...
def check_error_warm_start(early_stop_type, estimator_config, estimator):
    if check_is_pipeline(estimator):
        if (
            early_stop_type == EarlyStopping.WARM_START_ITER
            and f"{estimator.steps[-1][0]}__max_iter" in estimator_config
        ):
            raise ValueError(
                "tune-sklearn uses `max_iter` to warm "
                "start, so this parameter can't be "
                "set when warm start early stopping. "
            )

        if (
            early_stop_type == EarlyStopping.WARM_START_ENSEMBLE
            and f"{estimator.steps[-1][0]}__n_estimators" in estimator_config
        ):
            raise ValueError(
...
def is_tune_grid_search(obj):
    ...
    return (
        isinstance(obj, dict)
        and len(obj) == 1
        and "grid_search" in obj
        and isinstance(obj["grid_search"], list)
    )
    ...
    for log in loggers:
        if isinstance(log, str):
            if log == "tensorboard":
                ...
                raise ValueError(
                    f"{log} is not one of the defined loggers: " f"{defined_loggers}"
                )

...
from ._detect_booster import is_lightgbm_model
...
def resolve_early_stopping(early_stopping, max_iters, metric_name):
    if isinstance(early_stopping, str):
        ...
        raise TypeError(
            "`early_stopping` must be a str, boolean, "
...
class TuneBaseSearchCV(BaseSearchCV):
    ...
    def __init__(
        ...
        self.early_stop_type = get_early_stop_type(
            self.base_estimator, bool(early_stopping)
        )

        if not self._can_early_stop():
            if early_stopping:
                raise ValueError(
                    "Early stopping is not supported because "
        ...
        if not hasattr(self, "_is_multi"):
            _, self._is_multi = _check_multimetric_scoring(self.estimator, self.scoring)
    ...
    def _fit(self, X, y=None, groups=None, tune_params=None, **fit_params):
        ...
        if early_stopping:
            if not self._can_early_stop() and is_lightgbm_model(self.base_estimator):
                warnings.warn(
        ...
        if not hasattr(self, "_is_multi"):
            ...
            self.scoring_, _ = _check_multimetric_scoring(self.estimator, self.scoring)

        if self._is_multi:
            if not isinstance(self.refit, str) or self.refit not in self.scoring_:
                raise ValueError(
        ...
        if self.n_jobs < 0:
            ...
            if self.n_jobs < -1:
                warnings.warn(
                    "`self.n_jobs` is automatically set " "-1 for any negative values.",
                    category=UserWarning,
                )
        else:
        ...
        self.analysis_ = self._tune_run(
            X, y, config, resources_per_trial, tune_params, fit_params
        )
        ...
        if self.refit or not self._is_multi:
            ...
            if callable(self.refit):
                ...
                if self.best_index < 0 or self.best_index >= len(
                    self.cv_results_["params"]
                ):
                    raise IndexError("best_index index out of range")
            else:
                self.best_index = self.cv_results_[f"rank_test_{base_metric}"].argmin()
        ...
        if self.refit:
            ...
            self.best_estimator = clone(base_estimator.set_params(**self.best_params))
    ...
    def _format_results(self, n_splits, out):
        ...
        """
        trials = [trial for trial in out.trials if trial.status == Trial.TERMINATED]
        ...
        candidate_params = [self._clean_config_dict(config) for config in configs]
        ...
        results["time_total_s"] = np.array(
            [df["time_total_s"].to_numpy() for df in finished]
        ).flatten()

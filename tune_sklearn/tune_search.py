...
from .utils import check_is_pipeline, MaximumIterationStopper
from .tune_basesearch import TuneBaseSearchCV
from ._trainable import _Trainable, _PipelineTrainable
from .list_searcher import RandomListSearcher
from .utils import check_error_warm_start, resolve_logger_callbacks
...
def _check_distribution(dist, search_optimization):
    ...
    if search_optimization == "random":
        ...
        if search_optimization == "bayesian":
            ...
            if not isinstance(dist, ConfigSpace.hyperparameters.Hyperparameter):
                raise ValueError(
                    "distribution must be a tuple, list, or "
...
class TuneSearchCV(TuneBaseSearchCV):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            self._search_optimization_lower not in set(available_optimizations.values())
        ) and not isinstance(self._search_optimization_lower, Searcher):
            raise ValueError(
        ...
        if isinstance(self._search_optimization_lower, Searcher):
            if not hasattr(self._search_optimization_lower, "_mode") or not hasattr(
        ...
        if self._search_optimization_lower == "bohb" or isinstance(
            ...
            can_use_param_distributions = isinstance(
                check_param_distributions, CS.ConfigurationSpace
            )
            if isinstance(early_stopping, bool):
                if early_stopping is False:
                    raise ValueError("early_stopping must not be False when using BOHB")
                early_stopping = "HyperBandForBOHB"
            elif not isinstance(early_stopping, HyperBandForBOHB):
                if early_stopping != "HyperBandForBOHB":
                    warnings.warn(
                        "Ignoring early_stopping value, "
            ...
            raise ValueError(
                "search_optimization must be set to 'BOHB' "
        ...
        check_error_warm_start(self.early_stop_type, param_distributions, estimator)
    ...
    def _searcher_name(self):
        return available_optimizations.get(
            type(self._search_optimization_lower), self._search_optimization_lower
        )

    @property
    def _search_optimization_lower(self):
        return (
            self.search_optimization.lower()
            if isinstance(self.search_optimization, str)
            else self.search_optimization
        )
    ...
    def _get_bohb_config_space(self):
        ...
        for param_name, space in self.param_distributions.items():
            ...
            if isinstance(space, tuple) and len(space) >= 2 and len(space) <= 3:
                try:
                    low = float(space[0])
                    high = float(space[1])
                except Exception:
                    raise ValueError(
                        ...
                        f"are of type {type(low)} and {type(high)}"
                    ) from None
                if len(space) == 3:
                    prior = space[2]
                ...
                config_space.add_hyperparameter(
                    CS.CategoricalHyperparameter(name=param_name, choices=space)
                )
            else:
    ...
    def _get_optuna_params(self):
        ...
        for param_name, space in self.param_distributions.items():
            ...
            elif isinstance(space, BaseDistribution):
        ...
        for param_name, space in self.param_distributions.items():
            ...
            if isinstance(space, tuple) and len(space) >= 2 and len(space) <= 3:
                ...
                if len(space) == 3:
                    ...
                    if prior not in ["uniform", "log-uniform"]:
                        raise ValueError(
                            "prior needs to be either 'uniform' "
                ...
                if prior == "log-uniform":
                    ...
                    config_space[param_name] = hp.uniform(param_name, low, high)
            elif isinstance(space, list):
        ...
        if search_optimization == "bayesian":
            try:
                ...
                raise ImportError(
                    ...
                    "Do: pip install scikit-optimize"
                ) from None
        elif search_optimization == "bohb":
            try:
                ...
                raise ImportError(
                    ...
                    "is not installed. "
                    "Do: pip install hpbandster ConfigSpace"
                ) from None
        elif search_optimization == "hyperopt":
            try:
                ...
                raise ImportError(
                    "It appears that hyperopt is not installed. "
                    "Do: pip install hyperopt"
                ) from None
        elif search_optimization == "optuna":
            try:

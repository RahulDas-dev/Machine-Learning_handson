...
from .utils import EarlyStopping, _aggregate_score_dicts


def _replace_score_warning_details(
    warning: warnings.WarningMessage, new_traceback: str
):
...
class _Trainable(Trainable):
    ...
    def _early_stopping_ensemble(self, i, estimator, X_train, y_train):
        ...
        updated_n_estimators = (
            estimator.get_params()["n_estimators"] + self.resource_step
        )
            ...
            for i, (train, test) in enumerate(
                ...
                try:
                    if self.early_stop_type == EarlyStopping.PARTIAL_FIT:
                        self._early_stopping_partial_fit(i, estimator, X_train, y_train)
                        ...
                        self._early_stopping_ensemble(i, estimator, X_train, y_train)
                    else:
...
class _PipelineTrainable(_Trainable):
    ...
    def _setup_early_stopping(self):
        ...
        if self.early_stop_type == EarlyStopping.WARM_START_ITER:
            ...
            # epochs, while max_iters (which the user can set) is the usual
            # max number of calls to _trainable.
            self.estimator_config[f"{self.base_estimator_name}__warm_start"] = True
            ...
            self.estimator_config[f"{self.base_estimator_name}__warm_start"] = True
            self.estimator_config[f"{self.base_estimator_name}__n_estimators"] = 0
    ...
    def _early_stopping_partial_fit(self, i, estimator, X_train, y_train):
        ...
        if hasattr(estimator, "partial_fit"):
            ...
            if (
                "classes"
                in inspect.getfullargspec(estimator.steps[-1][1].partial_fit).args
            ):
    ...
    def _early_stopping_ensemble(self, i, estimator, X_train, y_train):
        ...
        updated_n_estimators = (
            estimator.get_params()[n_estimator_key] + self.resource_step
        )
